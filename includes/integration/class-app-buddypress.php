...
class Appointments_Integration_BuddyPress {
	...
	function bp_init() {
		...
		if ( function_exists( 'bp_loggedin_user_id' ) ) {
			...
			if (
				...
				wp_die( 'Du bist nicht befugt, dies zu tun.', 'appointments' );
				exit;
			}
	...
	function setup_nav() {
		...
		if ( ! appointments_is_worker( $user_id ) ) { $name = __( 'Mein Terminmanager', 'appointments' ); } else { $name = __( 'Mein Terminmanager als Dienstleister', 'appointments' ); }
		...
		if ( appointments_is_worker( $user_id ) && isset( $options['allow_worker_wh'] ) && 'yes' == $options['allow_worker_wh'] ) {
			bp_core_new_subnav_item(array(
				'name' => __( 'Terminmanager Einstellungen', 'appointments' ),
...
<input type="hidden" class="widefat" id="open_datepick" name="open[exceptional_days]" value="<?php echo $result['open']; ?>">
					<h4><?php _e( 'Meine Freien Tage', 'appointments' ); ?></h4>
...
<input type="hidden" class="widefat" id="closed_datepick" name="closed[exceptional_days]" value="<?php echo $result['closed']; ?>">
					...
					<div class="submit">
						<input type="submit" name="app_bp_settings_submit" value="<?php _e( 'Änderungen speichern', 'appointments' )?>" class="auto">
