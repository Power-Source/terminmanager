...
class App_MP_Bridge_Legacy {
	...
	function mp_buy_button_tag( $button, $product_id, $context ) {
		...
		$book_now = apply_filters( 'app_mp_book_now', __('Option wählen &raquo;','appointments') );
	...
	function mp_product_list_meta( $meta, $post_id) {
		...
		if ( in_array( $post_id, $this->mp_posts ) )
			return '<a class="mp_link_buynow" href="' . get_permalink($post_id) . '">' . __('Option wählen &raquo;', 'appointments') . '</a>';
		else
			return	$meta;
	}
