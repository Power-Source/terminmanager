		...
		return '<div class="app-settings-column_meta_info">' .
			...
			'<div>' .
				'<a href="#toggle" class="app-settings-column_meta_info-toggle" ' .
					'data-off="' . esc_attr( __( 'Mehr Information', 'appointments' ) ) . '" ' .
					'data-on="' . esc_attr( __( 'Weniger Information', 'appointments' ) ) . '" ' .
				'>' . __( 'Mehr Information', 'appointments' ) . '</a>' .
			'</div>' .
		'</div>';
	}
...
function appointments_maybe_cancel_appointment() {
	...
	if ( ! $app ) {
		wp_die( __( 'Der Termin existiert nicht.', 'appointments' ), __( 'Fehler', 'appointments' ), array( 'response' => 400 ) );
	}
	if ( $token != $app->get_cancel_token() ) {
		wp_die( __( 'Du darfst diese Aktion nicht ausführen.', 'appointments' ), __( 'Fehler', 'appointments' ), array( 'response' => 403 ) );
	}
	...
	if ( $url ) {
		...
		wp_die( __( 'Dein Termin wurde abgesagt!', 'appointments' ), __( 'Termin abgesagt', 'appointments' ), array( 'response' => 410 ) );
	}
}
