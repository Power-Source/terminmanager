<?php

class Appointments_Admin_Dashboard_Widget {

	public function __construct() {
		// Widget deaktiviert - Funktionalität ist jetzt im "Meine Termine" Widget integriert
		// add_action( 'wp_dashboard_setup', array( $this, 'register' ) );
	}

	public function register() {
		// Nicht mehr verwendet - siehe App_my_appointments_dashboard_widget
		/*
		if ( App_Roles::current_user_can( 'manage_options', App_Roles::CTX_DASHBOARD ) ) {
			wp_add_dashboard_widget( 'appointments', __( 'Terminmanager', 'appointments' ), array( $this, 'render' ) );
		}
		*/
	}

	public function render() {
		$app_counts = appointments_count_appointments();

		$num_active = $app_counts['paid'] + $app_counts['confirmed'];
		$num_pending = $app_counts['pending'];
		?>
		<div class="main">
			<ul>
				<li class="post-count">
					<span class="dashicons dashicons-clock"></span>
					<a href="<?php echo esc_url( self_admin_url( 'admin.php?page=appointments' ) ); ?>">
						<?php printf( _n( '%d Aktiver Termin', '%d Aktive Termine', $num_active ), $num_active ); ?>
					</a>
				</li>
				<li class="post-count">
					<span class="dashicons dashicons-backup"></span>
					<a href="<?php echo esc_url( self_admin_url( 'admin.php?page=appointments' ) ); ?>">
						<?php printf( _n( '%d Ausstehender Termin', '%d Ausstehende Termine', $num_pending ), $num_pending ); ?>
					</a>
				</li>
			</ul>
		</div>
		<?php
	}
}