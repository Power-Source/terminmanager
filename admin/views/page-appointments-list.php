		...
		<tfoot>
			<tr>
				<?php foreach ( $columns as $key => $title ): ?>
					...
					<?php if ( $key === 'cb' ): ?>
						<td id="cb" class="manage-column column-cb check-column">
							<label class="screen-reader-text" for="cb-select-all-2"><?php _e( 'Alle auswählen', 'appointments' ); ?></label>
						...
						<?php foreach ( $columns as $key => $value ): ?>
							<?php if ( $key === 'cb' ): ?>
								<th id="cb" class="column-delete check-column app-check-column" scope="row">
									<label class="screen-reader-text" for="cb-select-<?php echo $app->ID; ?>"><?php printf( __( 'Wähle Termin %d von %s', 'appointments' ), $app->ID, $app->get_formatted_start_date() ); ?></label>
			...
			<?php else: ?>
				<tr class="no-items">
					<td class="colspanchange" colspan="<?php echo count( $columns ); ?>">
						<?php _e('Es wurden keine Terminbuchungen gefunden.','appointments'); ?>
					</td>
				</tr>
			<?php endif; ?>
		</tbody>
	...
	<?php if ( 'removed' === $type ): ?>
		<div class="tablenav bottom">
			<div class="alignleft actions">
					...
					<p>
						<input type="submit" id="delete_removed" class="button-secondary" value="<?php _e('Ausgewählte Datensätze dauerhaft löschen', 'appointments') ?>" title="<?php _e('Durch Klicken auf diese Schaltfläche werden auf dem Server gespeicherte Protokolle gelöscht') ?>" />
...
<div class="tablenav bottom">
	<div class="alignleft actions">
		<form action="<?php echo admin_url('admin-ajax.php?action=app_export'); ?>" method="post">
			...
			<input type="submit" id="app-export-selected" class="app-export_trigger button-secondary" value="<?php esc_attr_e(__('Exportiere ausgewählte Terminbuchungen','appointments')); ?>" />
			<input type="submit" id="app-export-type" class="app-export_trigger button-primary" value="<?php esc_attr_e(sprintf(__('Exportiere %s Terminbuchungen','appointments'), App_Template::get_status_name($type))); ?>" data-type="<?php esc_attr_e($type); ?>" />
			<input type="submit" id="app-export-all" class="app-export_trigger button-secondary" value="<?php _e('Exportiere alle Terminbuchungen','appointments') ?>" title="<?php _e('Wenn Du auf diese Schaltfläche klickst, wird eine CSV-Datei mit ALLEN Termindatensätzen auf Deinem PC gespeichert','appointments') ?>" />
