...
function export_to_gcal ($result) {
	$.post(_app_admin_data.ajax_url, {
		...
		const remaining = parseInt(resp.remaining, 10),
			msg = resp.msg
		;
...
$(function () {
	const $link = $(".app-gcal-export_and_update");
	if (!$link.length) {return false;}
	
	const $root = $link.closest("td"),
...
$(document).on('click', '.notice-app-gdpr a, .notice-app-gdpr button', function() {
	const parent = $(this).closest( '.notice');
	const data = {
