...
class Appointments_Admin_Settings_Page {
	...
	public function __construct() {
		$this->page_id = add_submenu_page(
			'appointments',
			__( 'Terminmanager Einstellungen', 'appointments' ),
			__( 'Einstellungen', 'appointments' ),
	...
	public function get_tabs() {
		$tabs = array(
			'main'          => __( 'Basiseinstellungen', 'appointments' ),
			'working_hours' => __( 'Arbeitszeiten', 'appointments' ),
			'exceptions'    => __( 'Ausnahmen', 'appointments' ),
			'services'      => __( 'Services', 'appointments' ),
			'workers'       => __( 'Dienstleister', 'appointments' ),
			'addons'        => __( 'Erweiterungen', 'appointments' ),
	...
	public function get_sections() {
		$sections = array(
			'main' => array(
				'time' => __( 'Zeiten', 'appointments' ),
				'accesibility' => __( 'Barrierefreiheit', 'appointments' ),
				'display' => __( 'Anzeige', 'appointments' ),
				'payments' => __( 'Zahlungen', 'appointments' ),
				'notifications' => __( 'Benachrichtigungen', 'appointments' ),
				'advanced' => __( 'Erweitert', 'appointments' ),
				'gdpr' => __( 'Datenschutz', 'appointments' ),
			),
			'services' => array(
				'services' => __( 'Services', 'appointments' ),
				'new-service' => __( 'Erstelle neuen Service', 'appointments' ),
				'edit-service' => false,
			),
			'workers' => array(
		...
		if ( isset( $sections[ $tab ] ) ) {
			...
			wp_localize_script( 'app-settings', 'app_i10n', array(
				...
				'messages' => array(
					...
					'services' => array(
						'delete_confirmation' => __( 'Sicher das Du die gewählten Services löschen willst?', 'appointments' ),
					),
					'bulk_actions' => array(
						'no_items' => __( 'Wähle erst einige Services aus.', 'appointments' ),
						'no_action' => __( 'Wähle die auszuführende Aktion aus.', 'appointments' ),
					),
				),
			));
			return $content;
	...
	public function render() {
		...
		<div class="wrap appointments-settings">
			...
			<?php if ( isset( $_GET['updated'] ) ) :  ?>
				<div class="updated"><p><?php _e( 'Einstellungen gespeichert', 'appointments' ); ?></p></div>
			<?php endif; ?>
	...
	public function on_load() {
		...
		switch ( $tab ) {
			case 'services':
				...
				$option = 'per_page';
				$args = array(
					'label' => __( 'Anzahl der Services pro Seite', 'appointments' ),
	...
	private function _add_worker() {
		if ( empty( $_POST['services_provided'] ) ) {
			wp_die( __( 'Wähle mindestens einen Dienstleister aus', 'appointments' ) );
		}
